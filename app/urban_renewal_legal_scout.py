...
USAGE_RIGHTS_TOP_HTML = (
    '<strong>הבהרת שימוש וזכויות:</strong> '
    'לעו״ד רועי לביאב ולבאי כוחו ניתן אישור שימוש חופשי במערכת. '
    'כל שימוש אחר, הפצה, הטמעה או שינוי מסחרי דורשים אישור מראש מגיא לוזון. '
    'לפניות: <a href="mailto:guyluzon64@gmail.com">guyluzon64@gmail.com</a>'
)
USAGE_RIGHTS_FULL_MARKDOWN = """
**הבהרת שימוש וזכויות:**

המערכת נבנתה על ידי גיא לוזון כמתנת יום הולדת לעו״ד רועי לביאב. לעו״ד רועי לביאב ולבאי כוחו ניתן אישור שימוש חופשי במערכת לצורכי עבודה, בדיקה וחקירה ראשונית של מידע ציבורי בתחום ההתחדשות העירונית. כל שימוש אחר, העברה לצדדים נוספים, הטמעה, הפצה, שינוי מסחרי או שימוש שאינו במסגרת זו — דורש אישור מראש מגיא לוזון.

לפניות ואישור שימוש:
[guyluzon64@gmail.com](mailto:guyluzon64@gmail.com)
"""
USAGE_RIGHTS_FOOTER_HTML = (
    '© גיא לוזון — שימוש חופשי לעו״ד רועי לביאב ולבאי כוחו בלבד. '
    'שימוש אחר דורש אישור מראש: '
    '<a href="mailto:guyluzon64@gmail.com">guyluzon64@gmail.com</a>'
)
HEBREW_LABELS = {
...
UPLOAD_ML_WARNING = (
    "ML similarity score is calculated for uploaded records when the saved Notebook 03 model is available. "
...
.soft-card { background: #f7fbfa; border: 1px solid #dbe9e6; border-radius: 12px; padding: 1rem 1.1rem; margin: 0.45rem 0; color: #12312d !important; }
.warning-card { background: #fff7ed; border: 1px solid #fed7aa; border-radius: 12px; padding: 1rem 1.1rem; margin: 0.45rem 0; color: #7c2d12 !important; }
.success-card { background: #edfdf6; border: 1px solid #bbf7d0; border-radius: 12px; padding: 1rem 1.1rem; margin: 0.45rem 0; color: #14532d !important; }
.soft-card *, .warning-card *, .success-card * { color: inherit !important; }
...
.rights-card a, .rights-footer a { color: #0f766e !important; font-weight: 700; }
.rights-footer {
    margin-top: 1.5rem;
    padding: 0.8rem 1rem;
...
.upload-callout * { color: #164e63 !important; opacity: 1 !important; }
...
def load_ml_similarity_model():
    ...
    if not MODEL_PATH.exists() or not MODEL_METADATA_PATH.exists():
        return None, None, "ML model files not found."
    try:
...
def make_lawyer_note(status: Any, plan_number: Any, mavat_url: Any) -> str:
    ...
    if status == "PLAN_APPROVED":
        note = "נמצאה רשומת התחדשות עירונית רשמית. מופיע סטטוס תכנוני מאושר במקור הציבורי; מומלץ לבדוק מסמכי מבא״ת."
    elif status == "PERMIT_APPROVED":
        note = "נמצאה רשומה רשמית עם אינדיקציה לשלב אחרי רישוי; שווה בדיקה משפטית ותכנונית מעמיקה."
    elif status == "CONSTRUCTION":
        note = "הרשומה הרשמית מצביעה על שלב מימוש/ביצוע; יש לוודא סטטוס עדכני במבא״ת ובמערכות העירוניות."
    elif status == "PLAN_IN_PROGRESS":
        note = "נמצאה רשומת התחדשות רשמית. נראה שהתכנון עדיין מתקדם; כדאי לבדוק מסמכים וסטטוס ציבורי עדכני."
    else:
        note = "נמצאה רשומה רשמית, אך הסטטוס התכנוני לא ברור או חסר; נדרשת בדיקה ידנית."
    if has_value(plan_number):
        note += " קיים מספר תוכנית; מומלץ לפתוח מסמכי מבא״ת."
...
def score_uploaded_records(uploaded_standardized_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["project_momentum_explanation"] = "ציון זמני חושב בדאשבורד לפי סטטוס ציבורי, חוזק מקור, ביטחון דאטה והיקף פרויקט. נדרשת בדיקה ידנית."
...
def set_upload_ml_fallback(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["ml_score_warning"] = "ML similarity score could not be calculated for uploaded records."
    return ensure_columns(df)
...
def apply_saved_ml_similarity_model(uploaded_scored_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if load_error or model is None or metadata is None:
        st.warning(f"ההעלאה הצליחה, אבל ציון ה־ML לא חושב: {load_error or 'model unavailable'}")
    ...
    try:
        ...
        df["ml_model_used"] = metadata.get("model_name", "Saved Notebook 03 ML model")
        ...
        st.warning(f"ההעלאה הצליחה, אבל ציון ה־ML לא חושב: {type(exc).__name__}: {exc}")
        return set_upload_ml_fallback(df)
...
def process_uploaded_json_file(uploaded_file: Any) -> None:
    ...
    if file_signature == st.session_state.get("last_uploaded_file_signature"):
        st.info("הקובץ הזה כבר נטען בסשן הנוכחי. ידוע.")
        return
    ...
    st.success(f"שואג — נוספו {len(uploaded_scored_df):,} רשומות לדאשבורד בסשן הנוכחי.")
...
def build_case_summary(row: pd.Series) -> str:
    return "\n".join([
        ...
        f"סטטוס ציבורי: {row_text(row, 'planning_status_raw')}",
        f"סטטוס מנורמל: {row_text(row, 'planning_status_normalized')}",
        f"ציון מומנטום: {metric_value(row.get('project_momentum_score'), 1)}",
        f"רמת מומנטום: {row_text(row, 'project_momentum_label')}",
        f"קישור מבא״ת: {safe_url(row.get('mavat_url')) or 'לא זמין'}",
        f"קישור מפה: {safe_url(row.get('map_url')) or 'לא זמין'}",
        f"אזהרת דאטה: {row_text(row, 'data_quality_flag')}",
        "",
        "הערת עו״ד:",
        row_text(row, "lawyer_note", "אין הערה לעו״ד."),
        "",
        "הערת מערכת:",
        row_text(row, "dashboard_note", "אין הערת דאשבורד."),
        "",
        "הבהרה:",
        "המידע מבוסס על מקורות ציבוריים ואינו מחליף בדיקה משפטית/תכנונית או בדיקה במסמכי מבא״ת.",
    ])
...
def render_plan_review_checklist(row: pd.Series) -> None:
    ...
    with st.expander("📄 רשימת בדיקה לתכנית", expanded=False):
...
def render_record_detail(row: pd.Series, current_df: pd.DataFrame) -> None:
    ...
    if st.button("הוסף לתיק בדיקה זמני", key=f"add_to_review_basket_{record_id or 'unknown_record'}"):
        if record_id:
            ...
            st.success("שואג — המתחם נוסף לתיק הבדיקה הזמני.")
        else:
...
st.markdown('<div class="upload-callout">➕ להעלאת JSON חדש: פתח את הלשונית הראשונה "הוספת דאטה" או השתמש בכפתור ההעלאה שבסרגל הצד.</div>', unsafe_allow_html=True)

tab_upload, tab_search, tab_overview, tab_city, tab_scoring, tab_about = st.tabs(["➕ הוספת דאטה", "🔎 חיפוש מתחמים", "📊 תמונת מצב", "🏙️ סיכום לפי עיר", "🧠 הסבר ניקוד ו־ML", "⚖️ אודות ודיסקליימר"])

with tab_upload:
    st.markdown("### הוספת דאטה זמנית לסשן")
    st.write("""
כאן אפשר להעלות קובץ JSON רשמי מ־data.gov.il כדי לעדכן את הדאשבורד בסשן הנוכחי.

חשוב: ההעלאה אינה נשמרת קבוע. אם מרעננים את הדף, סוגרים את הדפדפן או שהאפליקציה מתאפסת — יהיה צורך להעלות את הקובץ שוב.

תאומר? ידוע. זה MVP שואג, לא מערכת ניהול מסדי נתונים.
""")
    st.caption("הדאטה הבסיסי של המערכת תמיד נטען אוטומטית. ההעלאה כאן מיועדת רק למקרה שבו יש קובץ חדש/מעודכן שרוצים לבדוק בדאשבורד.")
...
5. חוזרים לדאשבורד ומעלים את הקובץ בטופס למטה.

6. לאחר ההעלאה, הדאשבורד יחבר את הרשומות לדאטה הקיים ויעדכן את הטבלאות, הפילטרים והגרפים.
...
""")
    ...
    st.info("אם קיים מודל ML שמור מהמחברת השלישית, הדאשבורד יחשב גם מדד דמיון ML לרשומות שהועלו. המדד הוא אינדיקטיבי בלבד ואינו תחזית.")
    ...
    if st.button("איפוס לדאטה המקורי"):
        st.session_state["dashboard_df"] = base_df.copy()
        st.session_state["uploaded_session_active"] = False
        st.session_state["last_uploaded_file_signature"] = None
        st.success("הדאשבורד אופס לדאטה המקורי. ידוע.")
...
with st.sidebar.expander("➕ הוספת דאטה", expanded=False):
...
with tab_city:
    if st.session_state.get("uploaded_session_active"):
        ...
        st.caption("הסיכום מחושב מהדאטה המעודכן של הסשן הנוכחי.")
    else:
...
with tab_scoring:
    ...
    st.caption("רשומות שהועלו ידנית בסשן מקבלות Project Momentum Score, ואם קיים מודל שמור ממחברת 03 הן מקבלות גם ציון דמיון ML אינדיקטיבי.")
...
with tab_about:
    ...
    st.markdown("### הבהרת שימוש וזכויות")
