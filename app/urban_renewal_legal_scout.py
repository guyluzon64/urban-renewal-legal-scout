...
BUSINESS_TABLE_COLUMNS = [
    "city", "complex_name", "plan_number", "renewal_type", "planning_status_raw",
    "planning_status_normalized", "project_momentum_score", "ml_advancement_score",
    "confidence_level", "source_url", "mavat_url",
]
SUMMARY_COLUMNS = [
...
/* Keep complex Streamlit widgets theme-owned; avoid styling inner grid cells so scrolling is not clipped. */
...
def project_table_column_config() -> dict[str, Any]:
    return {
        "עיר": st.column_config.TextColumn("עיר", width="small"),
        "שם מתחם": st.column_config.TextColumn("שם מתחם", width="large"),
        "מספר תוכנית": st.column_config.TextColumn("מספר תוכנית", width="medium"),
        "מסלול": st.column_config.TextColumn("מסלול", width="medium"),
        "סטטוס תכנוני ציבורי": st.column_config.TextColumn("סטטוס תכנוני ציבורי", width="large"),
        "סטטוס מנורמל": st.column_config.TextColumn("סטטוס מנורמל", width="medium"),
        "Project Momentum Score": st.column_config.ProgressColumn(
        ...
        "Indicative Advancement Score": st.column_config.NumberColumn(
            "Indicative Advancement Score",
            min_value=0,
            max_value=100,
            format="%.1f",
            width="medium",
        ),
        "רמת בשלות אינדיקטיבית": st.column_config.TextColumn("רמת בשלות אינדיקטיבית", width="medium"),
        "רמת התקדמות אינדיקטיבית": st.column_config.TextColumn("רמת התקדמות אינדיקטיבית", width="medium"),
        "רמת אמינות נתונים": st.column_config.TextColumn("רמת אמינות נתונים", width="medium"),
        "דגל איכות נתונים": st.column_config.TextColumn("דגל איכות נתונים", width="medium"),
        "מקור מידע": st.column_config.LinkColumn("מקור מידע", width="medium", display_text="פתח מקור"),
        "קישור למבא״ת": st.column_config.LinkColumn("קישור למבא״ת", width="medium", display_text="פתח מבא״ת"),
        "קישור מפה": st.column_config.LinkColumn("קישור מפה", width="medium", display_text="פתח מפה"),
    }
...
st.markdown('<div class="upload-callout">לשימוש רגיל במערכת יש להתחיל בלשונית "חיפוש מתחמים". להעלאת JSON חדש ממקור ציבורי ניתן להשתמש בלשונית "הוספת נתונים" שבסוף סרגל הניווט או בכפתור ההעלאה שבסרגל הצד.</div>', unsafe_allow_html=True)

tab_search, tab_city, tab_overview, tab_scoring, tab_about, tab_upload = st.tabs(["חיפוש מתחמים", "סיכום לפי עיר", "תמונת מצב", "הסבר ניקוד ML", "אודות והבהרות", "הוספת נתונים"])

with tab_upload:
    st.markdown("### הוספת נתונים זמנית לסשן")
    st.info("אזור זה מיועד להעלאת קובץ נתונים חדש או בדיקת נתונים מתקדמת. לשימוש רגיל במערכת יש לעבור ללשונית חיפוש מתחמים.")
    st.write("""
...
with tab_search:
    st.markdown("### טבלת מתחמים ותוכניות")
    st.caption("הטבלה מציגה את הרשומות לאחר סינון. ניתן לעבור לתצוגה מלאה כדי לראות את כל העמודות, או להוריד את הנתונים לקובץ.")
    show_full_table = st.checkbox("תצוגת טבלה מלאה", value=False, help="מציג את כל עמודות הנתונים עם גלילה אופקית.")
    table_columns = filtered_df.columns.tolist() if show_full_table else BUSINESS_TABLE_COLUMNS
    ...
    st.dataframe(
        ...
        height=600,
        hide_index=True,
        column_config=project_table_column_config(),
    )
